...
class Piece {
  ...
  pieces: ChessPiece[]
  friendlyPieceList: ChessPiece[]
  enemyPieceList: ChessPiece[]
  constructor (coord: Point, pieces: ChessPiece[], camp: Camp, currentExecution: Camp) {
    const { x, y } = coord
    this.x = x
    this.y = y
    this.pieces = pieces
...
class Soldier extends Piece {
  ...
  redSoldierMove (): Array<[number, number]> {
    ...
    const isCrossRiver: boolean = this.y < 6
    if (isCrossRiver) {
  ...
  blackSoldierMove (): Array<[number, number]> {
    // 卒
    // 判断是否过河 没有过河的话就只有一个点位
    const isCrossRiver: boolean = this.y > 5
    if (isCrossRiver) {
      return this.soldierMove([[this.x + 1, this.y], [this.x - 1, this.y], [this.x, this.y + 1]])
    }
    return [[this.x, this.y + 1]]
  ...
  soldierMove (coordList: Array<[number, number]>): Array<[number, number]> {
...
class BigGun extends Piece {
  ...
  computedCanMove (): Array<[number, number]> {
    ...
    // 先列举出可以移动的地方
    // x轴
    const canMoveX: Array<[number, number]> = this.computedMoveX()
    // y轴
    const canMoveY: Array<[number, number]> = this.computedMoveY()
    return [...canMoveX, ...canMoveY]
  }

  computedMoveX (): Array<[number, number]> {
    const Xcoord: Array<[number, number]> = []
    for (let x = this.x - 1; x > 0; x--) {
      const hasPiece: ChessPiece | undefined = this.pieces.find(item => item.coord.x === x && item.coord.y === this.y)
      if (hasPiece) {
        break
      }
  ...
  computedMoveY (): Array<[number, number]> {
    const Ycoord: Array<[number, number]> = []
    for (let y = this.y - 1; y > 0; y--) {
      const hasPiece: ChessPiece | undefined = this.pieces.find(item => item.coord.x === this.x && item.coord.y === y)
      if (hasPiece) {
        break
      }
  ...
  computedCanEatX (): Array<[number, number]> {
    ...
    if (carriage.length > 0) {
      for (let x = carriage[0] - 1; x > 0; x--) {
        ...
        const hasPiece: ChessPiece | undefined = this.pieces.find(item => item.coord.x === x && item.coord.y === this.y)
        if (hasPiece) {
          const isEnemyPiece: boolean = hasPiece.camp !== this.currentExecution
          if (isEnemyPiece) {
            Xcoord.push([x, this.y])
          }
          break
...
class Elephant extends Piece {
  ...
  allMoveCoord (): Array<[number, number]> {
    ...
    const x = this.x
    const y = this.y
    const elephantRuleList: ElephantRule[] = [{
...
class Scholar extends Piece {
  ...
  redScholarMove (): Array<[number, number]> {
    ...
    const blackScholarCoord = map.find(item => item.xCoord === this.x)
  ...
  blackScholarMove (): Array<[number, number]> {
    ...
    const redScholarCoord = map.find(item => item.xCoord === this.x)
...
class Boss extends Piece {
  ...
  allMoveCoord (): Array<[number, number]> {
    ...
    const bossCoord = map.find(item => item.xCoord === this.x)
    const bossCanMoveCoord: Array<[number, number]> = bossCoord ? bossCoord.coord : []
  ...
  computedCanEat (): Array<[number, number]> {
    ...
    const specialEatCoord = this.camp === Camp.RED ? this.canEatJiang() : this.canEatShuai()
    return [...this.allMoveCoord().filter(item => {
      return this.enemyPieceList.find(e => e.coord.x === item[0] && e.coord.y === item[1])
    }), ...specialEatCoord]
  }
  ...
  canEatJiang (): Array<[number, number]> {
    const pieceCoord: Array<[number, number]> = []
    for (let y = this.y - 1; y > 0; y--) {
